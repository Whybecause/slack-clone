...
const generateCode = () => {
  const code = Array.from(
    { length: 6 },
    () =>
      "0123456789abcdfghijklmnopqrstuvwxyz"[Math.floor(Math.random() * 36)]
  ).join("");
...
export const join = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return workspace?._id as Id<"workspaces">;
  }
})
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Also delete in this workspace: members, channels, conversations, messages, reactions
